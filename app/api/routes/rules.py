...
def list_rules(db: Session = Depends(get_db)):
    ...
    return MonitoringRuleListResponse(rules=rules, total=len(rules))
...
def create_rule(rule_data: MonitoringRuleCreate, db: Session = Depends(get_db)):
    """
...
def update_rule(
    rule_id: UUID, rule_data: MonitoringRuleUpdate, db: Session = Depends(get_db)
):
    """
